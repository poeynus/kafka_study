...
파티션 수와 컨슈머 수 1대1 매핑이 제일 좋다.

**컨슈머그룹**

카프카는 메시지를 기본 값으로 7일 보관, 하나의 토픽을 2갸의 컨슈머그루 빝우서 가져갈 수 있음

## 세그먼트

key 값을 기준으로 마지막 값만을 보존 - 요구사항에 따라 compact 옵션 사용

**RACK Awareness**

동일한 RACK에 브로커들이 위치하면 RACK 장애 시 커다란 장애 발생

RACK별 균형을 유지하여 RACK 장애 시 장애 최소화 - 클라우드 환경에서 AZ도 비슷함

대부분 분산 시스템에서 자주 사용되는 옵션

**버전 업그레이드**

중단 업그레이드, 무중단 업그레이드(롤링 업그레이드)
